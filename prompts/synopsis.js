 ...
 * @returns {string}
 */
function buildRoleSynopsisPrompt(role, company, insights, mode = 'premium', topGap = null, companyUrl = '', roleUrl = '', agentPromo = null) {
...
Headcount Growth % (last 6 months) — negative means headcount shrank, positive means it grew: ${hg6m}`.trim();
  ...
  let stepCounter = 3;
  const promoLine = agentPromo
    ? `${stepCounter++}. Include this sentence verbatim: "${agentPromo}"\n`
    : '';
  if (canAskMore) stepCounter++;
  const endingStep = agentPromo ? stepCounter : (canAskMore ? 4 : 3);
...
${canAskMore ? `3. ${canAskMore}\n` : ''}${promoLine}${endingStep}. ${endingInstruction}
